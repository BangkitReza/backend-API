...
const login = async(req, res) => {
  ...
  try {
      ...
      res.json({ success: true, msg:'Login berhasil', data: {
          uid: user.uid, 
          email: user.email
      } });
  } catch (error) {
      res.status(404).json({succes: false, msg:'Login tidak berhasil'});
  }
}
...
const predictionStunt = async (req, res) => {
  try {
    ...
    const predictionsBinary = prediction[0][0] > 0.5 ? 1 : 0;
    ...
    res.status(200).json({
      ...
      predictionsBinary,
      msg: 'Riwayat prediksi berhasil disimpan',
    });
  } catch (error) {
...
const getAllPrediction = async (req, res) => {
  try {
    ...
    const predictionQuery = query(predictionCollection, where('userId', '==', currentUserId));
